...
class AdvancedMLOptimizer:
    ...
    # Performance tracking decorator removed for compatibility
    ...
    async def _linear_prediction(self, features: dict[str, float], metric: str) -> tuple[float, float]:
        ...
        for sample in self.performance_history:
            if metric in sample['performance']:
                feature_vector = [sample['features'].get(key, 0.0) for key in features]
        ...
        try:
            ...
            # Make prediction
            feature_vector = [1.0] + [features.get(key, 0.0) for key in features]
    ...
    async def _calculate_feature_importance(
        ...
        if len(self.performance_history) < 20:
            return {key: 1.0 / len(features) for key in features}
    ...
    async def get_current_optimal_config(self, context: dict[str, Any]) -> dict[str, Any] | None:
