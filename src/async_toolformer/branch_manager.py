...
class CancellationStrategy:
    ...
    custom_scorer: Callable[[ToolResult], float] | None = None
...
class BranchManager:
    ...
    def __init__(
        self,
        strategy: CancellationStrategy | None = None,
    ...
    async def get_branch_status(self, branch_id: str) -> BranchStatus | None:
        """Get the status of a branch."""
        if branch_id in self._branches:
            return self._branches[branch_id].status
        return None

    async def get_branch_result(self, branch_id: str) -> ToolResult | None:
